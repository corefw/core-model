...
class MysqlModel extends BaseModel {
	...
	_createWhereFromParameters( request ) {
		...
		_.each( fm, function ( dbFieldName, paramKey ) {
			...
			if ( _.startsWith( paramKey, "page[" ) ) {

				return true;
			}

			// ignore sort parameter
	...
	getMany( request ) {
		...
		queryConfig.args.where = me._createWhereFromParameters( request );
		queryConfig.args.order = me._createOrderFromParameters( request );
	...
	getOne( request ) {
		...
		return me._executeDbQuery( queryConfig ).then( function ( result ) {
			...
			me.$log(
				...
				"getOne operation complete: [" + result.length + "] " +
				"records returned."
			);

			return me._createResource( result );
		} );
	}
	 ...
	 *     `Sequelize.Model`.
	 * @param {Object} cfg.args - Arguments passed to the DB query.
