...
class MysqlModel extends BaseModel {
	 ...
	 * @param {Array.<Relationship.BaseRelationship>} [modelConfig.relationships] - Relationships.
	...
	_initialize( modelConfig ) {
		...
		if ( TIPE( modelConfig.dbFieldMappings.attributes ) !== "object" ) {

			modelConfig.dbFieldMappings.attributes = {};
		}

		if ( TIPE( modelConfig.dbFieldMappings.meta ) !== "object" ) {

			modelConfig.dbFieldMappings.meta = {};
		}

		if ( TIPE( modelConfig.dbFieldMappings.relationships ) !== "object" ) {

			modelConfig.dbFieldMappings.relationships = {};
		}
	...
	_connectToDatabase() {
		...
		me.$log(
			"debug",
			"Connecting to MySQL ('" + me.mysqlHostname + "') as user " +
			"'" + me.mysqlUsername + "'."
		);
	...
	_createValuesFromData( request ) {
		...
		let data = request.body.data.attributes;
		...
		_.each( fm, function ( dbFieldName, dataKey ) {
			...
			if ( data[ dataKey ] ) {
				...
				values[ dbFieldName ] = data[ dataKey ];
			}
		} );
	...
	getMany( request ) {
		...
		me.$log(
			"debug",
			"Starting a " + me.$className + "#getMany() query operation."
		);
		...
		return me._executeDbQuery( queryConfig ).then( function ( result ) {
			...
			me.$log(
				"info",
				"model.operation.getmany.complete",
				"getMany operation complete: " +
	...
	getOne( request ) {
		...
		me.$log(
			"debug",
			"Starting a " + me.$className + "#getOne() query operation."
		);
		...
		return me._executeDbQuery( queryConfig ).then( function ( result ) {
			...
			let count	= 1;
			let data	= me._createResource( result );

			me.$log(
				"info",
				"model.operation.getone.complete",
				"getOne operation complete: [" + count + "] " +
				"records returned."
			);
	...
	patchOne( request ) {
		...
		// Build Query Arguments
		queryConfig.values = me._createValuesFromData( request );
		queryConfig.options.where = me._createWhereFromParameters( request );

		me.$log(
			"debug",
			"Starting a " + me.$className + "#patchOne() query operation."
		);
		...
		return me._executeDbQuery( queryConfig ).then( function ( result ) {
			...
			me.$log(
				"info",
				"model.operation.patchone.complete",
				"patchOne operation complete: [" + result[ 0 ] + "] " +
				"records updated."
			);

			return me.getOne( request );
		} );
	}
	...
	deleteOne( request ) {
		...
		return me._executeDbQuery( queryConfig ).then( function ( result ) {
			...
			me.$log(
				...
				"deleteOne operation complete: [" + result[ 0 ] + "] " +
				"records deleted."
			);
